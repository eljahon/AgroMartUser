<template>
  <div v-if="!loading" class="overflow-hidden grid grid-cols-1 gap-4 xl:grid-cols-6">
    <div
      class="
        responsive
        flex
    ...
    >
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <router-link v-if="windowWidth < 768" :to="{ path: localePath('/chats') }">
            ...
            <div
              ...
              "
            >
              <div v-for="(mes, index) in messages" :key="index" class="mb-2 flex flex-col">
                <div
                  v-if="mes.senderID ? (mes.senderID.id == currentUser.id ? true : false) : false"
                  ...
                  >
                    <div class="bg-indigo-300 mb-1">
                      <img
                        ...
                        :src="`https://admin.agsat.uz${mes.filePath}`"
                      >
                    </div>
                    <div @click.prevent.stop="handleClick($event, mes)">
                      {{ mes.text }}
                    </div>
                  </div>
                </div>
      ...
      >
        <div class="w-full mb-3 h-12">
          <div class="bottom-0 right-0 left-0 text-gray-400">
            <div class="flex bg-gray-100 rounded-md items-center px-4 mb-2 py-1">
              <div class="flex mt-2 items-center" @click="openMediaModal">
                <label for="chatFile">
                  <div class="text-2xl -mt-1 ml-2 cursor-pointer text-gray-500 hover:text-blue-400">
      ...
      <div v-else class="relative px-5 pb-5 bg-white z-0">
        <div class="align-middle text-center">
          ...
          <span v-if="state === 'trading'" class="rounded-md py-1 px-2 bg-green-200 text-gray-600">
            {{ $t('text.chatRoomClosed') }}
          </span>
        </div>
      </div>
          ...
          <div class="text-center self-center">
            ...
            <div class="flex justify-center">
              <p class="text-gray-500 text-sm">
                <span class="text-base text-gray-600 font-medium">4.7</span> (256 ta baholash)
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="relative bg-white z-0 mt-3">
        <div class="border rounded-md shadow-md p-3">
          <div class="grid grid-cols-3 gap-3">
            <div v-for="(video, index) in videos" :key="index" class="flex my-3">
              <iframe :src="video.url" frameborder="0" allowfullscreen class="" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1">
            ...
            >
              <router-link
                :to="{ path: localePath(`/useful-information/${info.id}`), query: { id: info.id } }"
...
export default {
      ...
      this.socketDisconnector().then(() => {
        this.fetchData().then(() => {
          this.fetchCurrentRoom().then(() => {
            if (this.currentRoom.isCompleted === true && this.currentRoom.rate0to5 === null) {
  ...
  methods: {
    ...
    async fetchCurrentRoom () {
      if (this.$route.query.room_id !== 'new') {
        await this.$store
          .dispatch('crud/chats/room/getRoomsById', { id: this.$route.query.room_id })
          .then((res) => {
            this.currentRoom = res
          })
      }
    ...
    async fetchData () {
      if (this.state === 'consultant') {
        await this.$store
          .dispatch('crud/user/getUsersById', { id: this.$route.query.consultant_id })
          .then((res) => {
            this.consultant = res
          })
        await this.$store
      ...
      if (this.state === 'trading') {
        await this.$store
          .dispatch('crud/trading/getTradingById', { id: this.$route.query.product_id })
          .then((res) => {
            this.product = res
          })
      }
