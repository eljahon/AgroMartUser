<template>
  <div class="max-w-7xl mx-auto py-6 lg:px-8">
    ...
    <div v-if="!loading" class="container max-w-4xl mx-auto p-4">
      <div class="grid md:grid-cols-2 grid-cols-1 gap-6">
        <div class="grid grid-cols-4 gap-2">
          <div class="col-span-4">
            <img
              v-if="productDetail.gallery.length > 0"
              :src="$tools.getFileUrl(productDetail.gallery[0])"
              class="h-60 w-full object-cover rounded-sm"
            >
            <img
              v-else-if="productDetail.mainimage"
        ...
        <div>
          <p class="md:text-3xl text-xl text-gray-700">
            {{ productDetail.title[$i18n.locale] }}
          </p>
          <div class="flex items-center justify-between mt-2 space-x-3">
            ...
            >
              {{ productDetail.price }} {{ $t('word.sum') }} <span v-if="productDetail.unit">/ {{ productDetail.unit.namejson[$i18n.locale] }} </span>
            </p>
            ...
            <div v-if="productDetail.typename">
              <p
                ...
                "
              >
                {{ productDetail.typename[$i18n.locale] }}
              </p>
              <p
                v-else
                class="
          ...
          <div class="grid grid-cols-2 gap-5 mt-7">
            ...
            <div class="bg-gray-100 rounded-md font-medium text-gray-700 space-x-7 text-sm p-2 flex col-span-2">
              ...
              <div class="block space-y-2">
                <div>{{ $t('word.all') }}: {{ productDetail.count }} <span v-if="productDetail.unit">/ {{ productDetail.unit.namejson[$i18n.locale] }} </span></div>
...
export default {
  ...
  methods: {
    ...
    async fetchDetail () {
      await this.$store.dispatch('crud/trading/getTradingById', this.$route.query).then((res) => {
      ...
      if (this.isLoggedIn) {
        this.$store
          .dispatch('crud/chats/room/postRooms', {
            data: {
              consultantID: this.productDetail.userid.id,
              userID: this.currentUser.id,
              istradingchat: true,
              tradingpost: this.productDetail
            }
          })
          .then((res) => {
            ...
            this.$router.push({
              ...
              query: { room_id: res.id, product_id: this.productDetail.id }
            })
          })
      } else {
        this.$router.push({
