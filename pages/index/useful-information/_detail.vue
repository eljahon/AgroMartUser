    ...
    <div v-if="!loading">
      <div class="max-w-3xl mx-auto">
        <h3 class="text-gray-700 font-medium md:text-4xl text-lg">
          {{ informationDetail.title[$i18n.locale] }}
        </h3>
        <p class="text-green-500 md:text-base text-sm pt-3">
          {{ informationDetail.description[$i18n.locale] }}
        </p>
        <div class="mt-4 flex items-center justify-between">
          <div class="flex">
            <img v-if="informationDetail.consultantid !== null" class="h-10 w-10 object-cover rounded-full" :src="$tools.getFileUrl(informationDetail.consultantid.avatar)">
            <img v-else class="h-10 w-10 object-cover rounded-full" src="https://silkozari.com/web/images/miscellaneous/avatar.jpg">
            <div class="block ml-4">
              <p v-if="informationDetail.consultantid !== null" class="text-sm text-gray-600">
                {{ `${informationDetail.consultantid.name ? informationDetail.consultantid.name : ''} ${informationDetail.consultantid.surname ? informationDetail.consultantid.surname : ''}` }}
              </p>
              <div class="flex justify-between space-x-1 text-xs text-gray-500 pt-2">
                <div>
                  <time datetime="2020-03-16">
                    {{ $tools.getDateTime(informationDetail.created_at) }}
                  </time>
                </div>
              </div>
            </div>
        ...
        <p v-if="informationDetail.content[$i18n.locale] !== 'None'" class="my-3 text-base text-gray-700" v-html="informationDetail.content[$i18n.locale]" />
        <div v-if="informationDetail.attachment && informationDetail.attachment[$i18n.locale] !== 'None'" class="py-5">
          <div class="mt-2 sm:flex sm:items-start sm:justify-between">
            <div class="max-w-xl text-base text-gray-700">
              <p>{{ informationDetail.title[$i18n.locale] }}</p>
            </div>
            <div class="mt-5 sm:mt-0 sm:ml-6 sm:flex-shrink-0 sm:flex sm:items-center">
              <a :href="$tools.getFileUrl(informationDetail.attachment[$i18n.locale])" target="_blank">
