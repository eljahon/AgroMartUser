<template>
  <div class="max-w-7xl mx-auto py-6 px-4 lg:px-8">
    <main class="flex-1 relative overflow-y-auto focus:outline-none">
      <div v-show="data.trading.length > 0" class="mt-3 mb-2">
        <router-link :to="localePath('/trading')" class="md:flex hidden text-gray-600 font-medium text-2xl">
          {{ $t('text.tradingFloor') }}
        </router-link>
        <div id="trading">
      ...
      <div v-show="data.news.length > 0" class="mt-5 mb-2">
        <router-link :to="localePath('/news')" class="md:flex hidden text-gray-600 font-medium text-2xl">
          {{ $t('text.lastNews') }}
        </router-link>
        <div id="news">
          <div class="grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-4">
            <div v-for="(info, index) in data.news" :key="index">
              <nuxt-link v-show="info.title[$i18n.locale] !== 'None'" :to="{ path: localePath(`/news/${info.id}`), query: {id: info.id}}">
                <div class="col-span-3 bg-white flex">
                  <div class="rounded-md border hover:shadow-lg shadow-md w-full">
                    ...
                    <div class="p-2">
                      ...
                      <p class="text-xs pt-2 line-clamp-2 text-green-500">
                        {{ info.category.title[$i18n.locale] }}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <div v-show="data.useful.length > 0" class="mt-5 mb-2">
        <router-link :to="localePath('/useful-information')" class="md:flex hidden text-gray-600 font-medium text-2xl">
              ...
              <div v-show="info.title[$i18n.locale] !== 'None'" class="col-span-3 h-auto p-2 bg-white rounded-md hover:shadow-lg shadow-md border">
                <router-link :to="{ path: localePath(`/useful-information/${info.id}`), query: {id: info.id}}">
                  <div class="grid grid-cols-3">
                    <div class="">
                      <img v-if="info.mainphoto || info.thumbnail" :src="$tools.getFileUrl(info.thumbnail ? info.thumbnail : info.mainphoto)" class="w-40 h-28 object-cover shadow-lg rounded-md">
